...
def setup_profile():
    try:
        # Get user ID from JWT token and convert from string to int
        ...
        print(f"Raw profile setup data: {request.data}")
        print(f"Parsed profile setup data: {data}")
        print(f"JWT Identity: {jwt_identity} (converted to user_id: {user_id})")
        current_app.logger.info(f"Profile setup request for user {user_id}: {data}")
        ...
        current_app.logger.error(f"Profile setup error: {str(e)}")
